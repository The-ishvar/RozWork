...
import { Bell, Briefcase, Globe, Home, ImageIcon, LogOut, Menu, Moon, ShieldCheck, SunMedium, UserRound, Users, X } from 'lucide-react'
...
const Navbar = () => {
  ...
  const { language, supportedLanguages, setLanguage, toggleLanguage, t } = useLanguage()
  ...
  const bottomNavItems = [
    ...
    { action: toggleLanguage, label: language.toUpperCase(), icon: Globe },
    { to: user ? '/profile' : '/login', label: user ? t('nav.profile') : t('common.login'), icon: user ? UserRound : UserRound },
  ]
        ...
        <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-2">
            ...
            <nav className="hidden items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-300 md:flex">
              ...
              <NavLink to="/gallery" className="flex items-center gap-1 rounded-full px-3 py-2 hover:bg-slate-100 hover:text-blue-600 dark:hover:bg-slate-800"> <ImageIcon size={16} /> {t('nav.gallery')}</NavLink>
              {user ? <NavLink to="/dashboard" className="flex items-center gap-1 rounded-full px-3 py-2 hover:bg-slate-100 hover:text-blue-600 dark:hover:bg-slate-800"> <Bell size={16} /> {t('nav.notifications')}</NavLink> : null}
              {user ? <NavLink to="/profile" className="flex items-center gap-1 rounded-full px-3 py-2 hover:bg-slate-100 hover:text-blue-600 dark:hover:bg-slate-800"> <UserRound size={16} /> {t('nav.profile')}</NavLink> : null}
            </nav>
              ...
              <div className="hidden items-center gap-2 sm:flex">
                {supportedLanguages.map((lang) => (
                  <button
                    ...
                    className={`rounded-full px-3 py-2 text-sm font-semibold transition ${language === lang.code ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                  >
          ...
          {isMenuOpen ? (
            <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 shadow-sm md:hidden dark:border-slate-700 dark:bg-slate-800">
              <div className="flex flex-col gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                ...
                <NavLink to="/gallery" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2 hover:bg-white dark:hover:bg-slate-700"> <ImageIcon size={16} /> {t('nav.gallery')}</NavLink>
                {user ? <NavLink to="/dashboard" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2 hover:bg-white dark:hover:bg-slate-700"> <Bell size={16} /> {t('nav.notifications')}</NavLink> : null}
                {user ? <NavLink to="/profile" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2 hover:bg-white dark:hover:bg-slate-700"> <UserRound size={16} /> {t('nav.profile')}</NavLink> : null}
                {user ? (
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200 bg-white/95 backdrop-blur md:hidden dark:border-slate-700 dark:bg-slate-900/95">
        <div className="mx-auto flex max-w-6xl items-center justify-around px-2 py-2">
          {bottomNavItems.map((item, index) => {
            ...
            return (
              <button key={`lang-${index}`} type="button" onClick={item.action} className="flex flex-1 flex-col items-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold text-slate-500 transition hover:text-blue-600 dark:text-slate-400">
                <Icon size={18} />
                <span>{item.label}</span>
              </button>
            )
